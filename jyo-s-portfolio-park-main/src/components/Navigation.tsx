...
const Navigation = () => {
    // State to control nav bar visibility on mobile
              ...
              return (
                <Link key={link.path} to={link.path} className="relative">
                  <motion.div
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="flex items-center gap-2 px-3 py-2 rounded-lg text-black dark:text-white bg-transparent hover:bg-transparent transition-all"
                  >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.aside
            ...
            className="fixed top-0 left-0 h-screen w-4/5 max-w-xs bg-background/90 dark:bg-background/80 backdrop-blur-xl shadow-xl z-[60] md:hidden flex flex-col text-foreground dark:text-foreground"
          >
